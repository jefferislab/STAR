...
#'returns a simulating function from a \code{gssanova} fitted model. Ogata's
#'thinning simulation method is used. Functions \code{maxIntensity},
#'\code{mkSelf} and \code{mkMappedI} are utility functions for the first two.
#'Function \code{mkPostSimAnalysis} returns a function analysing a simulated
#'spike train. Functions \code{mkSimFct} and \code{mkPostSimAnalysis} return
#'functions which can in principle be safely used in parallel applications,
#'that is, they have everything they need in their closure.
#'
#'Function \code{thinProcess} simulates a spike train with Ogata's thinning
#'method (Ogata, 1981). The latter method required the maximal intensity of the
#'process to be known. If such is not the case, that is, if argument
#'\code{intensityMax} is left \code{missing}, a proposed maximal intensity is
#'obtained with function \code{maxIntensity}. If during the simulation an
#'actual intensity larger than the given \code{intensityMax} occurs, the
#'simulation is interrupted and an error message is generated.
#'
#'Function \code{maxIntensity} uses the central point of the variable space as
#'its intial guess. The "BFGS" method of \code{\link{optim}} is used to find
#'the maximal intensity.
#'
#'Function \code{mkPostSimAnalysis} uses function
#'\code{\link[codetools]{findGlobals}} in order to find among the functions
#'called by \code{dfFct} the ones which are defined in the global environment.
#'These functions are copied in the environment (Gentleman and Ihaka, 2000) of
#'the function returned by \code{mkPostSimAnalysis}. If the global environment
#'defined function called by \code{dfFct} \emph{do not} call themselves over
#'functions defined in the global environment, the returned function can be
#'safely used as argument \code{fun} of package \code{snow}'s
#'\code{clusterApply} function.
#'
#'@aliases thinProcess maxIntensity mkSelf mkMappedI mkSimFct mkPostSimAnalysis
#'@param object A \code{\link[gss]{ssanova}} or a \code{\link[gss]{ssanova0}}
#'object.
#'@param m2uFctList A list of functions. There should be as many functions as
#'there are "internal" variables in \code{object}. An internal variable is a
#'variable whose value is changed by the occurrence of a spike, like the
#'elapsed time since the last spike of the simulated neuron, the duration of a
#'former inter spike interval of a given lag, etc. The names of the components
#'(functions) of the list should be the names of the variables.  Each function
#'should correspond to the map to uniform function used before fitting the
#'model.
#'@param m2uSelf The map to uniform function used to transform the actual
#'elapsed time since the last spike values before fitting the model.
#'@param m2uI The map to uniform function used to transform the actual former
#'ISI durations before fitting the model.
#'@param lag The considered lag (integer > 0).
#'@param trueData A data frame containing the "true data" of the simulated
#'epoch. This is to ensure that "external" variables such as the elapsed time
#'since the last spike of a functionally coupled neuron are available.
#'@param formerSpikes A vector of previous spike times. This is to make the
#'computation of former inter spike intervals possible in every case.
#'@param intensityMax The value of the maximal intensity. If missing function
#'\code{maxIntensity} is called to estimate it.
#'@param dfWithTime A data frame with one variable named "time". The latter
#'variable is used to obtain the bin width with which the original spike train
#'was discretized.
#'@param stList The list of \code{spikeTrain} objects with one of the trains
#'partly simulated. A single (partly simulated) \code{spikeTrain} object can
#'also be used.
#'@param trainNumber An integer, the index of the modeled and simulated spike
#'train in \code{stList}.
#'@param timeWindow A numeric vector of length 1 or 2. This argument specifies
#'the time domain over which the fits contained in argument objects was
#'performed. It is implicitly assumed that the (partial) simulation was
#'performed outside this time domain. When a vector of length 1 is used the
#'fitting time domain is taken as \code{c(0,timeWindow)}.
#'@param objects A list of \code{\link[gss]{ssanova}} or
#'\code{\link[gss]{ssanova0}} objects. Each element of the list is a "model"
#'with which analysis will be performed. A single \code{\link[gss]{ssanova}} or
#'\code{\link[gss]{ssanova0}} object can also be used.
#'@param dfFct A function whose argument is a the same as the first argument of
#'function \code{\link[STAR]{mkGLMdf}} and which returns a data frame suitable
#'for use as argument \code{newdata} in \code{\link[gss]{predict.ssanova}}.
#'@param \dots Additional arguments passed to \code{\link{optim}} which is
#'called internally with the "BFGS" method in function \code{maxIntensity}. In
#'functions \code{thinProcess} and \code{mkSimFct}, additional argument passed
#'to function \code{maxIntensity} if necessary.
#'@return \code{thinProcess} returns a \code{spikeTrain} object.
#'
...
#'
#'\code{mkSimFct} returns a function simulating a \code{spikeTrain} object. The
#'simulation is done with function \code{thinProcess}. The returned function
#'takes no argument. The maximal intensity required by the thinning method is
#'stored in the closure of the returned function.
#'
#'\code{mkPostSimAnalysis} returns a function taking a \code{spikeTrain} object
#'as its single argument. This function returns a list of lists. Each list
#'correspond to one of the models in argument \code{objects}. Each sub list has
#'two components: \code{lpp} (the log predictive probability) and \code{ttt}
#'(the time transformed train, a CountingProcessSamplePath object).
#'@note These functions are designed to implement a rather specific type of
#'analysis which is exposed in the "big STAR tutorial" available at:
#'\url{http://sites.google.com/site/spiketrainanalysiswithr/}. The exemple
#'below shows a "complete" analysis, more details and other exemples can be
#'found in the big tutorial.
#'@author Christophe Pouzat \email{christophe.pouzat@@gmail.com}
#'@seealso \code{\link[gss]{gssanova}}, \code{\link{as.spikeTrain}},
#'\code{\link{mkGLMdf}}, \code{\link{mkCPSP}},
#'@references Gentleman, R. and Ihaka, R. (2000) Lexical Scope and Statistical
#'Computing. \emph{Journal of Computational and Graphical Statistics} \bold{9}:
#'491-508.
#'
#'Ogata, Y. (1981) On Lewis' simulation method for point processes. \emph{IEEE
#'Transactions on Information Theory} \bold{IT-29}: 23-31.
