#'Compute the Log Probability of a "New" Data Set Using a Fitted Model
#'Prediction
#'
#'This function is designed to select models by cross-validation. If two models
#'A and B managed to pass the Ogata's tests, one should split the data set in
#'two parts, part1 and part2 fit each model on each part to get four fitted
#'model objects: A1, A2, B1, B2. The chosen model should then be the one giving
#'the largest of: \code{predictLogProb(A1,part2) + predictLogProb(A2,part1)}
#'and \code{predictLogProb(B1,part2) + predictLogProb(B2,part1)}.
#'
...
#'
#'Where \code{event[i]} is 0 or 1 depending on the absence or presence of a
#'spike at the considered time.  \emph{A binomial regression is assumed here}.
#'
#'@param object an object inheriting from \code{\link[gss]{ssanova}} and
#'\code{\link[gss]{ssanova0}} (\code{\link[gss]{gssanova}} and
#'\code{\link[gss]{gssanova0}} objects are therefore suitable).
#'@param newdata a data frame containing the required variables. This data
#'frame \emph{must be different from the one used to obtain} \code{object}.
#'@return A numeric, the sum over the index \code{i} above, the log probability
#'of the data contained in \code{newdata} assuming that the model contained in
#'\code{object} is correct.
#'@author Christophe Pouzat \email{christophe.pouzat@@gmail.com}
#'@seealso \code{\link[STAR]{mkGLMdf}}, \code{\link[STAR]{quickPredict}}
