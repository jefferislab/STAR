...
#'intended for use in batch processing situations where a decision to stop with
#'the current model or go on with a more complicated one must be made
#'automatically.
#'
#'\code{summary.transformedTrain} computes summary statistics corresponding to
#'plot 1, 2 and 5 of \code{\link{plot.transformedTrain}}.
#'
#'The first plot tests the uniformity of the spikes (transformed) times on the
#'(transformed) observation window using a KS test. If the ecdf of the
#'(transformed) times is within the 95\% band then the first element of
#'component \code{uniformOnTTime} of the returned list is set to \code{TRUE}.
#'It is set to \code{FALSE} otherwise. The second component is relative to the
#'99\% band.
#'
#'The second plot tests the exponential distribution of the intervals between
#'successive spikes transformed times. Again if the empirical curve stays
#'within the 95, respectively 99\%, confidence band, the first, respectively
#'second, element of component \code{BermanTest} of the returned list is set to
#'\code{TRUE}. It is set to \code{FALSE} otherwise.
#'
#'The fifth plot tests that the variance is equal to the length of the
#'(transformed) observation time for \code{object}, using point-wise CI. If n
#'different observation times are defined over the whole observation window, we
#'expect (1 - CI/100)*n points to be out with an approximate binomial
#'distribution. For each CI defined (95 and 99\%, by default), component
#'\code{VarTime} of the returned list contains the probability of observing a
#'number as large as or smaller than the one observed under the binomial null
#'hypothesis.
#'
#'@param object a \code{\link{transformedTrain}} object.
#'@param \dots additional arguments passed to \code{\link{varianceTime}}.
#'@return A \code{list} with the following 3 components:
#'@returnItem uniformOnTTime A two named components vector of boolean.
#'@returnItem BermanTest A two named components vector of boolean.
#'@returnItem VarTime A named component vector with as many components as
#'passed to \code{\link{varianceTime}} via the \code{...} argument with
#'p-values of a binomial distribution.
#'@author Christophe Pouzat \email{christophe.pouzat@@gmail.com}
#'@seealso \code{\link{transformedTrain}}, \code{\link{plot.transformedTrain}},
#'\code{\link{mkGLMdf}}
